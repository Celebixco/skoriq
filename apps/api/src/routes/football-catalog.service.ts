...
export class FootballCatalogService {
  ...
  async getTeamProfile(teamId: string): Promise<FootballTeamProfile> {
    ...
    const deduplicatedRecentRows = recentMatches.filter((row) => {
      const oppId = row.home_team_id === teamId ? row.away_team_id : row.home_team_id;
      const dateStr = row.kickoff_at ? new Date(row.kickoff_at).toISOString().slice(0, 10) : "";
      const key = `${dateStr}-${oppId}`;
    ...
    const deduplicatedUpcomingRows = upcomingMatches.filter((row) => {
      const oppId = row.home_team_id === teamId ? row.away_team_id : row.home_team_id;
      const dateStr = row.kickoff_at ? new Date(row.kickoff_at).toISOString().slice(0, 10) : "";
      const key = `${dateStr}-${oppId}`;
    ...
    return {
      ...
      recentMatches: deduplicatedRecentRows.map((row) => mapTeamProfileRecentMatchRow(row, teamId)),
      upcomingMatches: deduplicatedUpcomingRows.map((row) => mapTeamProfileUpcomingMatchRow(row, teamId)),
      playerAvailability,
