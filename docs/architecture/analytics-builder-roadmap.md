...
Manual pre-match pipeline runner status: `pipeline:football:prematch` orchestrates the existing match-based team form builder, H2H feature builder, match prediction feature builder, reasoning builder, candidate generator, consistency check, and optional draft persistence for one match or one reviewed/enabled league scope. Dry-run is the default and writes no rows. Execute mode is manual only, requires a safe local/Neon-test DB target, selects only matches inside `now -> now + 24h` and outside the minimum lead-time cutoff, and persists drafts only through `--persist-draft --enforce-window` when the feature snapshot is `ready` and at least one non-blocked recommendation candidate exists. It does not call providers, run ingestion, publish, mark member-visible, settle, or create tahmin kombini output.
...
- Future pre-match automation should run periodically and process only near-term upcoming matches in the next 24 hours by default.
